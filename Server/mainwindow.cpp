...
QSound sound("D:\\QT_program\\qt-tcp-streamer-master\\qt-tcp-streamer-master\\Server\\sound\\sintel_trailer-audio.wav");
...
void MainWindow::Display(int index)
{
    ...
    if (index > 0 && index <= 870)
    {
        qDebug()<< noImg <<endl;
        QPixmap img = QPixmap("../Video/" + noImg + ".jpg");
    ...
    {
        ui->lblVideo->setPixmap(QPixmap("D:\\QT_program\\qt-tcp-streamer-master\\qt-tcp-streamer-master\\Server\\img\\sintel-title.jpg").scaled(QSize(1280,720),Qt::KeepAspectRatio));
    }

    // Convert actual time to actual speed of the video
    actualTime = QString::number(index / (int)(_speed * 24));
    // Change time to fit with speed
    totalTime = QString::number((870 / (int)(_speed * 24)) + 1);
...
void MainWindow::Tick()
{
    ...
    if (_imgIndex == 871) // If the last file is about to be played
    {
